...
class Chunk(object):
	...
	def __init__(self, video_file, position_file, min_positions, max_positions, position_rel_indexes, frame_rel_indexes, validation_rate, shuffle):
		...
		self.frames_training = list(range(-min(frame_rel_indexes), round(self.num_frames * (1.0-validation_rate)) ))
		self.frames_validation = list(range(max(self.frames_training)+1, self.num_frames))
	...
	def get_frame(self, index, transformer = None):
		...
		# Prepare the frames
		with self.lock:
			frames = np.zeros(shape=(len(self.frame_rel_indexes), self.height, self.width, 3), dtype=np.float32)
			count = 0
	...
	def get_next_training_frame(self, transformer = None):
		...
		if self.current_frame_training < len(self.frames_training):
			...
			(frames, output) = self.get_frame( self.frames_training[self.current_frame_training], transformer)
	...
	def get_next_validation_frame(self, transformer = None):
		...
		if self.current_frame_validation < len(self.frames_validation):
			# Load the sequence of frames
			(frames, output) = self.get_frame(self.current_frame_validation, transformer)
...
class TrainingInput(object):
	def __init__(self, transformer, settings_file, position_rel_indexes, frame_rel_indexes, valdiation_rate, shuffle_frames=True, shuffle_chunks=True):
		...
		for row in f.readlines():
			...
			chunk = Chunk(os.path.join(self.base_path, tokens[0]), os.path.join(self.base_path, tokens[1]), min_range, max_range, position_rel_indexes, frame_rel_indexes, valdiation_rate, shuffle_frames)
	...
	def get_next_training_frame(self):
		if self.active_chunk < len(self.chunks):
			...
			(frames, output) = self.chunks[self.chunk_order[self.active_chunk]].get_next_training_frame(self.transformer)
			reset_memory = False
	...
	def get_next_validation_frame(self):
		if self.active_chunk < len(self.chunks):
			...
			(frames, output) = self.chunks[self.chunk_order[self.active_chunk]].get_next_validation_frame(self.transformer)
			reset_memory = False
