...
class Chunk(object):
    ...
    def __init__(self, video_file, position_file, min_positions, max_positions, position_rel_indexes, frame_rel_indexes, validation_rate, shuffle, keep_in_memory = False, transformer = None):
        ...
        # Load all normalized video frames into memory
        self.keep_in_memory = keep_in_memory
        self.memory_video_frames = None
        if self.keep_in_memory:
            print("Loading normalized frames into memory...")

            # Load all the frames
            self.memory_video_frames = np.zeros(shape=(self.num_frames, self.input_height, self.input_width, 3), dtype=np.float32)

            ret, frame = self.reader.read()
            index = 0
            while ret == True and frame is not None:
                # Add this frame
                self.memory_video_frames[index,:,:,:] = normalize_frame(frame)[:,:,:]

                index += 1
                ret, frame = self.reader.read()

            if index != self.num_frames:
                print("WARNING! When loading frames into memory, only %i of %i were loaded." % (index, self.num_frames))
    ...
    def get_frame(self, index):
        ...
        for idx, rel_idx in enumerate(self.frame_rel_indexes):
            if self.keep_in_memory:
                frames[idx, :, :, :] = self.memory_video_frames[index + rel_idx, :, :, :]
                count += 1
            else:
                with self.lock:
                    # Read the frame off disk and normalize it
...
class VideoTransform():
    ...
    def __init__(self, zoom_range=0.1, rotation_range=20, width_shift_range=0.1, height_shift_range=0.1, shear_range= 0.1, color_rescale_range=0.3, color_offset_range=0.3, brightness_rescale_range=0.3, brightness_offset_range=0.3, white_noise=0.05, gaussian_noise=0.1, fill_mode='nearest', vertical_flip=False, horizontal_flip=True, horizontal_flip_invert_indices = [3,4,5], horizontal_flip_reverse_indices = [0,1,2], data_format=None, random_crop=None, crop=None, resize=None):

        # Random image colour shifts
        self.color_rescale_range = color_rescale_range
        self.color_offset_range = color_offset_range
        self.brightness_rescale_range = brightness_rescale_range
        self.brightness_offset_range = brightness_offset_range
    ...
    def _process_frame(self, frame, output):
        ...
        # Apply the additive brightness offsets and additive noise
        max_color = np.max(x)
        min_color = np.min(x)
        range = max_color - min_color

        # Apply the specific color channel rescales
        color_offsets = prng.uniform(-self.color_offset_range * range, self.color_offset_range * range, (3))
        color_rescales = 1.0 + prng.uniform(-self.color_rescale_range, self.color_rescale_range, (3))
        for i in [0,1,2]:
            x[:,:,i] = (x[:,:,i] + color_offsets[i]) * color_rescales[i]

        # Apply the overall frame brightness changes
        total_offset = prng.uniform(-self.brightness_offset_range * range, self.brightness_offset_range * range)
        level = prng.uniform(0, self.white_noise * range)
        white_noise = prng.uniform(-level, level, x.shape)
        std_dev = prng.uniform(0, self.gaussian_noise * range)
        gaussian_noise = prng.normal(0, std_dev, x.shape)
        brightness_rescale = 1.0 + prng.uniform(-self.brightness_rescale_range, self.brightness_rescale_range)

        x = (x + total_offset + white_noise + gaussian_noise) * brightness_rescale
...
class TrainingInput(object):
    def __init__(self, transformer, settings_file, position_rel_indexes, frame_rel_indexes, valdiation_rate, shuffle_frames=True, shuffle_chunks=True, keep_in_memory=False):
        ...
        for row in f.readlines():
            ...
            chunk = Chunk(os.path.join(self.base_path, tokens[0]), os.path.join(self.base_path, tokens[1]), min_range, max_range, position_rel_indexes, frame_rel_indexes, valdiation_rate, shuffle_frames, keep_in_memory, transformer)
    ...
    def reset(self):
        self.active_chunk = 0
        self.active_chunk_validation = 0
    ...
    def get_next_validation_frame(self):
        if self.active_chunk_validation < len(self.chunks):
            # Get the next validation frame from the active chunk
            (frames, output) = self.chunks[self.active_chunk_validation].get_next_validation_frame()
            reset_memory = False
            ...
            if frames is None:
                ...
                self.active_chunk_validation += 1
                if self.active_chunk_validation < len(self.chunks):
                    self.chunks[self.active_chunk_validation].reset()
