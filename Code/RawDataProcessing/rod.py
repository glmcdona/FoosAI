...
class Rod():
	def __init__(self, box, name, gap_colour_edge = (22,28,39),  gap_bar_colour_threshold = 70, gap_min_size = 40, gap_bar_width = 5, line_detection_frequency = 30, rod_left = None, rod_right = None, model = None):
		self.model = model
		self.box = box
		self.name = name
	...
	def update_rod_line(self, edges):
		if self.model is not None:
			return True # Model is used for tracking
		
		if self.rod_left is not None:
			return True # A defined line is used for the rod
	...
	def _normalize_frame(self, frame):
		rod_region = self.get_rod_region(frame)
		
		# Crop the frame, resample, and normalize
		frame_resized = cv2.resize(rod_region, (self.model["width"], self.model["height"]), interpolation=cv2.INTER_AREA)
		...
		# Build the frame
		data = np.zeros(shape=(1, 1, self.model["height"], self.model["width"], 3), dtype=np.float32)
		data[0,0,:,:,:] = np.ascontiguousarray(norm_image)
		
		return data
	
	def blackout(self, frame):
		# Blackout this rod's region
		black = frame
		black[self.box[1]:(self.box[1] + self.box[3]), self.box[0]:(self.box[0] + self.box[2]),:] = 0
		return black
	
	def get_rod_region(self, frame):
		# Gets the part of the frame for this rod
		return frame[self.box[1]:(self.box[1] + self.box[3]), self.box[0]:(self.box[0] + self.box[2])]
	...
	def find_gap_size(self, key_frames):
		if self.model is not None:
			return 0 # Model is used for tracking
